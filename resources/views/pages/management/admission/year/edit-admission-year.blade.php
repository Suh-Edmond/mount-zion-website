    ...
    <div class="py-2">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="sm:p-8 bg-white shadow sm:rounded-lg">
                <form method="post"
                    action="{{ route('manage.admission.years.update', ['slug' => $currentSession->slug]) }}">
                    @csrf
                    @method('PUT')

                    <h2 class="text-lg font-medium text-gray-900">
                        {{ __('Update Admission Session') }}
                    </h2>

                    <p class="mt-1 text-sm text-gray-600">
                        {{ __("Provide Information below to update an admission session.") }}
                    </p>
                    ...
                    <div class="my-5">
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                            value="{{$currentSession->name}}" required autocomplete="name" />
                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                    </div>

                    <div class="my-5">
                        <x-input-label for="year" :value="__('Year')" />
                        <input type="number" id="year" value="{{$currentSession->year}}" required name="year" min="2020"
                            max="2050"
                    ...
                    <div class="my-5">
                        ...
                        <input type="date" id="start_date" value="{{$currentSession->start_date}}" required
                            name="start_date" min="2020" max="2050"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-500 focus:border-blue-500 p-2">
                    ...
                    <div class="my-5">
                        ...
                        <input type="date" id="end_date" value="{{$currentSession->end_date}}" required name="end_date"
                            min="2020" max="2050"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-500 focus:border-blue-500 p-2">
                    ...
                    <div class="my-5">
                        ...
                        <select id="school_id" name="school_id"
                            ...
                            <option selected value="{{$currentSession->program->id ?? ''}}">
                                {{$currentSession->program->school->name ?? ''}}
                            </option>
                        </select>
