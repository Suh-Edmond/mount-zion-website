<x-modal name="edit-event-speaker{{$speaker->id}}" :show="$errors->slotCreation->isNotEmpty()" focusable x-data="">
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-1 gap-10 mt-4">
                <img class="w-25 rounded" src="{{asset($speaker->picture)}}" alt="Blog Image" height="50px">
            </div>
            <div class="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-3 gap-10 mt-4">
                <div>
                ...
                <form method="post"
                    action="{{ route('manage.events.speakers.store', ['slug' => $speaker->event->slug]) }}"
                    ...
                    <div class="grow my-4">
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                            value="{{$speaker->name}}" required autocomplete="name" />
