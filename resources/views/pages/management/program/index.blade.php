    ...
    <div class="max-w-7xl mx-auto bg-white shadow-sm sm:rounded-lg py-5 px-5 mt-4 ">
        <div class=" py-4 my-5 ">
            ...
            <p class="mt-1 text-sm text-gray-600">
                {{ __('List of programs under this school') }}
            </p>
        </div>
                ...
                <div class="m-5 p-5 flex justify-end">
                    <nav aria-label="Page navigation example">
                        <ul class="flex items-center -space-x-px h-10 text-base">
                            <li  class="{{$programs->currentPage() == 1 ? 'page-item disabled':'page-item'}}">
                                <a href="{{route('manage.academics.programs.list', ['page' =>$programs->currentPage() - 1, 'slug' => $school->slug])}}" class="{{$programs->currentPage() == 1? 'cursor-not-allowed flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white':'flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'}}">
                            ...
                            @for($i = 1; $i <= $programs->lastPage(); $i++)
                                <li>
                                    <a href="{{route('manage.academics.programs.list', ['page' => $i, 'slug' => $school->slug])}}" class="{{$programs->currentPage() == $i ?'flex items-center justify-center px-4 h-10 leading-tight text-white bg-blue-800 border border-blue-800 hover:bg-blue-800 hover:text-white dark:bg-blue-800 dark:border-blue-800 dark:text-white dark:hover:bg-blue-800 dark:hover:text-white' : 'flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'}}">
                                        {{$i}}
                                    </a>
                                </li>
                            @endfor

                            <li class="{{$programs->currentPage() == $programs->lastPage() ? 'page-item disabled': 'page-item'}}">
                                <a href="{{route('manage.academics.programs.list', ['page' =>$programs->currentPage() + 1, 'slug' => $school->slug])}}" class="{{$programs->currentPage() == $programs->lastPage() ? 'cursor-not-allowed flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'
