        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
            ...
            <div class="rounded overflow-hidden shadow-lg flex flex-col">
                ...
                <div class="relative"><a href="{{route('manage.academics.programs.show', ['slug'=> $program->slug])}}">
                        <img class="w-full" src="{{asset($program->image_path)}}" alt="Blog Image" height="50px">
                        <div
                ...
                <nav aria-label="Page navigation example">
                    <ul class="flex items-center -space-x-px h-10 text-base">
                        <li class="{{$programs->currentPage() == 1 ? 'page-item disabled':'page-item'}}">
                            <a href="{{route('manage.academics.programs.list', ['page' =>$programs->currentPage() - 1, 'slug' => $school->slug])}}"
                                class="{{$programs->currentPage() == 1? 'cursor-not-allowed flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white':'flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'}}">
                        ...
                        @for($i = 1; $i <= $programs->lastPage(); $i++)
                            ...
                            <li
                                class="{{$programs->currentPage() == $programs->lastPage() ? 'page-item disabled': 'page-item'}}">
                                <a href="{{route('manage.academics.programs.list', ['page' =>$programs->currentPage() + 1, 'slug' => $school->slug])}}"
                                    class="{{$programs->currentPage() == $programs->lastPage() ? 'cursor-not-allowed flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white'
